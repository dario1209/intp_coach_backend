...
export async function coachChat(req: Request, res: Response) {
  try {
    ...
    // Inject INTP system prompt
    const fullMessages = [
      { role: 'system' as const, content: intpPrompts.system + '\n\n' + userContext },
      ...messages,
    ];
...
export async function coachPlan(req: Request, res: Response) {
  try {
...
export async function coachNudge(req: Request, res: Response) {
  try {
